...
https://github.com/torrvision/focal_calibration/blob/475422a6754e20d25216158a874749c33e826e21/evaluate.py
'''
...
class AdaptiveECELoss(nn.Module):
    ...
    def forward(self, softmaxes, labels, confidences=None, predictions=None):
        # softmaxes = F.softmax(logits, dim=1)
...
# def evaluate_cal(logits, labels):
def evaluate_cal(scores, labels, bins=15):
    ...
    ece_criterion = ECELoss(n_bins=bins)
    adaece_criterion = AdaptiveECELoss(n_bins=bins)
    cece_criterion = ClasswiseECELoss(n_bins=bins)

    p_ece = ece_criterion(scores, labels).item()
    p_adaece = adaece_criterion(scores, labels).item()
    p_cece = cece_criterion(scores, labels).item()
    # p_nll = nll_criterion(scores, labels).item()
    p_nll = nll_criterion(torch.log(scores), labels).item()
