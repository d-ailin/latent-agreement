...
def evaluate(probs, y_true, verbose = False, normalize = False, bins = 15):
    ...
    # y_pred_correct = y_true.reshape(-1) == np.argmax(probs, axis=1)
    # brier = brier_score_loss(y_true=y_true, y_prob=y_prob_true)  # Brier Score (MSE)
    # brier = brier_score_loss(y_true=y_pred_correct, y_prob=y_prob_true)  # Brier Score (MSE)
    # brier = 0
...
class TemperatureScalingWithOthers():
    ...
    # Find the temperature
    def fit(self, logits, true, ssl, kwargs={}, x0=None):
        """
        Trains the model and finds optimal temperature
        ...
        ssl_size = ssl.shape[1]

        if x0 is None:
            x0 = np.ones(ssl_size+1)/2

        print('x0', x0)

        opt = minimize(self._loss_fun, x0 = x0, args=(logits, true, ssl), options={'maxiter':self.maxiter, 'disp':True}, method = self.solver, **kwargs)
    ...
    def predict(self, logits, ssl, temp = None, return_logit=False):
        """
        Scales logits based on the temperature and returns calibrated probabilities
        ...
        temp_ssl = (temp[0] + np.sum(temp[1:] * ssl, -1) ).reshape(-1, 1)

        temp_ssl = np.abs(temp_ssl)

        assert np.all(temp_ssl >= 0)

        # or change to mulitple
        if return_logit:
            return logits * temp_ssl
        return softmax(logits * temp_ssl)


def cal_results(fn, res, m_kwargs = {}, approach = "all", return_probs=False):
    ...
    if return_probs:
        return evaluate(probs_test, y_test, verbose=False, normalize=True), probs_test
